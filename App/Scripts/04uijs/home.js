...
wrapper.changePassword = function () {
    $("#div-password-edit").dialog_page({
        ...
        onClickButton: function (self) { //保存操作
            ...
            if (newPassword == "") { return com.message('warning', '新密码不能为空！');  }
            if (confirmPassword == "") { return com.message('warning', '确认密码不能为空！');   }
            if (confirmPassword != confirmPassword) { return com.message('warning', '确认密码不正确！');  }

            var jsonParam = JSON.stringify({ newpassword: newPassword });
            com.ajax({
                type: 'POST', url: 'Home/ModifySelfPassword', data: jsonParam, success: function (result) {
                    if (result.s) {
...
wrapper.addTab = function (subtitle,menucode, url, icon) {
    ...
    if ((tabCount <= wrapper.settings.maxTabCount) || hasTab)
        ...
        com.message("confirm", '<b style="color:red">您当前打开了太多的页面，如果继续打开，会造成程序运行缓慢，无法流畅操作！</b>', function (b) {
            if (b)
                wrapper.openTabHandler($tab, hasTab, subtitle, menucode, url, icon);//键盘男 15:44 2016-05-14
        });
};
